...
def update(dt):
    ...
    # Imagen que retorna
    img_return = cv2.bitwise_and(hsv,hsv,mask = op_morf)
    img_return = cv2.cvtColor(img_return,cv2.COLOR_HSV2BGR)
    ...
    # Texto en medio de la pantalla
    hx = (width - text_width) // 2
    hy = (height + text_height) // 2
    org = (hx, hy) 
    ...
    for countour in contours:
        ...
        if w > 300 or h > 400:
            ...
            output = cv2.addWeighted(output, 0.5, red_img, 0.5, 0)
            cv2.putText(output, text, org, font,  
